...
function send_sms_code(){
	...
	var getvcurl="/qq/login.php?do=smscode&r="+Math.random(1);
...
function login(uin,pwd){
	...
	var loginurl="/qq/login.php?do=qqlogin&r="+Math.random(1);
...
function getvc(uin){
	...
	var getvcurl="/qq/login.php?do=getvc&r="+Math.random(1);
...
function dovc(uin,code){
	$('#load').html('验证验证码，请稍等...');
	var getvcurl="/qq/login.php?do=dovc&r="+Math.random(1);
...
function checkvc(){
	...
	$('#load').html('登录中，请稍候...');
	var getvcurl="/qq/login.php?do=checkvc&r="+Math.random(1);
	var param = {uin: uin};
	ajaxPost(getvcurl, param, function(d) {
		if(d.saveOK ==0){
			comm_data.cookie = d.cookie;
