...
class WeiboLogin
{
	...
	private function weibosso($ticket){
		...
		if (isset($arr['result']) && $arr['result']==true) {
			...
			return array('code' => -1, 'msg' => '登录成功，获取微博cookie失败（'.$arr['reason'].'）');
		} else {
			return array('code' => -1, 'msg' => '登录成功，获取微博cookie失败');
		}
	}

	public function getBasicInfo($cookie){
		$cookies = '';
		foreach($cookie as $key=>$value){
			$cookies .= $key.'='.$value.'; ';
		}
		$url = 'https://www.weibo.com/ajax/setting/getBasicInfo';
		$data = $this->get_curl($url, 0, $this->referrer, $cookies);
		$arr = json_decode($data, true);
		return $arr;
	}
