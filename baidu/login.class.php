...
class baiduLogin{
	...
	public function login($time,$user,$pwd,$p,$vcode=null,$vcodestr=null,$BAIDUID=null){
		...
		$data = $this->get_curl($url,0,$this->referrer,$cookie,1,1);
		preg_match("/pplogid=(.*?);/", $data['header'], $pplogid);
	...
	public function checkvc($user){
		...
		preg_match("/BAIDUID=(.*?);/", $data['header'], $BAIDUID);
		$arr=json_decode($data['body'],true);
	...
	public function getphone($phone){
		...
		$data=$this->get_curl($url,$post,$this->referrer,0,1,1);
		preg_match("/BAIDUID=(.*?);/", $data['header'], $BAIDUID);
		$arr=json_decode($data['body'],true);
		if($arr['errInfo'] && $arr['errInfo']['no']=='0'){
			return array('code'=>0,'msg'=>$arr['errInfo']['msg'], 'BAIDUID'=>$BAIDUID[1]);
		}else{
			return array('code'=>$arr['errInfo']['no'],'msg'=>$arr['errInfo']['msg'], 'BAIDUID'=>$BAIDUID[1]);
		}
	}
	...
	public function sendsms($phone,$BAIDUID,$vcode=null,$vcodestr=null,$vcodesign=null){
		...
		$post='username='.$phone.'&tpl=tb&clientfrom=&countrycode=&gid=E528690-4ADF-47A5-BA87-1FD76D2583EA&dialogVerifyCode='.$vcode.'&vcodesign='.$vcodesign.'&vcodestr='.$vcodestr.'&vstrswitch=1&time='.time();
		$data=$this->get_curl($url,$post,$this->referrer,$cookie);
		$arr=json_decode($data,true);
		if($arr['errInfo'] && $arr['errInfo']['no']=='0'){
